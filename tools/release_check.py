...
def run_release_check(write_report: str = "", strict: bool = False, quiet: bool = False) -> dict:
    ...
    if not quiet:
        ...
        print(f"  provider_semantics_label: {provider_semantics.get('provider_semantics_label')}")
        print(f"  primary_provider_id: {provider_semantics.get('primary_provider_id')}")
        print(f"  primary_contract_id: {provider_semantics.get('primary_contract_id')}")
        print(f"  provider_candidate_count: {provider_semantics.get('candidate_count')}")
        print(f"  provider_runtime_policy: {provider_semantics.get('runtime_policy')}")
        print(f"  provider_fallback_enabled: {provider_semantics.get('fallback_enabled')}")
