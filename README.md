...
- 展示模式：基金观察池 / 原始板块
- 主题口径：严格代表口径 / 代表口径 / 广度观察，仅在基金观察池模式下显示
- 板块类型：行业资金流 / 概念资金流
- 刷新间隔：15 秒 / 30 秒 / 60 秒
- 净流入 Top N
- 净流出 Top N
...
基金观察池不是简单求和，而是提供三种口径：

- `strict_representative / 严格代表口径`：只使用主题核心板块的精确匹配；如果没有核心板块，才使用精确匹配的相关板块作为替代，并在页面中标记。它是默认口径，最克制，适合默认观察。
...
基金观察池做了主题归并。严格代表口径只看核心板块精确匹配，代表口径在必要时用近似板块补充，广度观察会纳入更多相关板块。它们都不是基金净值，也不是严格行业资金流总量，只用于辅助观察主题热度和方向。
